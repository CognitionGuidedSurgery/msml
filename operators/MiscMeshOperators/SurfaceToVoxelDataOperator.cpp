...
namespace MSML
{
  ...
  std::string SurfaceToVoxelDataOperator(const char* infile, const char* outfile, float accuracy_level, float smoothing)
  {
    ...
    vtkSmartPointer<vtkImageData> image = MiscMeshOperators::ImageCreateWithMesh(pd, 300); // arguments: (vtkPolydata, resolution)
    // Note: this is properly tuned only for the mitral valve. The higher the resolution value (e.g. 500), the thinner the valve leaflets.
