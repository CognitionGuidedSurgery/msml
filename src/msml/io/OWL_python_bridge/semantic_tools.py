...
class OntologyParser(object):
    def __init__(self, ontologyName):
        ...
        #manage imports
        importList = self._ontoGraph.subjects(RDF.type, OWL.Ontology)

        if importList:
            self._importedGraph = Graph()
            (head, tail) =  path.split(ontologyName)
            importFilename = path.join(head,'MSMLBaseOntology.owl')
            self._importedGraph.parse(importFilename)



    def createPythonModule(self, directory, import_list=list()):
        ...
        for moduleName in modulesDict:
            ...
            for impMod in import_list:
                try:
                    currentLoadedModule = importlib.import_module('..'+ impMod, 'msml.model.generated.subpkg')
                    for name, obj in inspect.getmembers(currentLoadedModule):
                        if inspect.isclass(obj):
                            establishedClasses.add( obj.__name__)
                #import statement in generated module
                    currentModuleString += 'from msml.model.generated.'+impMod+' import *\n\n'
                except ImportError:
                    pass
    ...
    def parseAttributes(self, className):
        ...
        for property in propList:
            ...
            if propertyType == OWL.DatatypeProperty:
                ...
                theClassName = propertyURI.getClassName()
                if theClassName:
                    if theClassName.startswith('for') or theClassName.startswith('has'):
                        theClassName = theClassName[3:]

                    returnDict[theClassName] = 1
                ...
                if cardinality is None:
                    ...
                    if cardinality is None:
                        ...
                        if self._importedGraph is not None:
                           rangeBNode = self._importedGraph.value( property, RDFS.range, None )
                           cardinality = self._importedGraph.value( rangeBNode, OWL.qualifiedCardinality, None )
                           if cardinality is None:
                              cardinality = self._importedGraph.value( rangeBNode, OWL.minQualifiedCardinality, None )
                        if cardinality is None:
                ...
                if theClassName:
                    ...
                    returnDict[theClassName] = cardinality
